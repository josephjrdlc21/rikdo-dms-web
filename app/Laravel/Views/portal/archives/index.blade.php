    ...
    <form method="GET" action="">
        <div class="card-header">
            ...
            <div class="card-header-action">
                ...
                <a href="{{route('portal.archives.index')}}" class="btn btn-sm btn-secondary" style="border-radius: 0.25rem !important;">Reset Filter</a>
            </div>
        </div>
        <div class="card-body">
            <div class="row">
            ...
            <table class="table table-striped mb-0">
                ...
                <tbody>                         
                    ...
                    <tr>
                        ...
                        <td>
                            {{$researches->research_type->type}} / 
                            <span class="badge badge-{{Helper::research_badge_status($researches->status)}}">{{Helper::capitalize_text(str_replace('_', ' ', $researches->status))}}</span>
                        </td>
                        <td>{{$researches->department->dept_code ?? 'N/A'}}</td>
                        <td>{{$researches->course->course_code ?? 'N/A'}}<br><small>{{$researches->yearlevel->yearlevel_name ?? ''}}</small></td>
                        <td>{{$researches->submitted_by->name ?? 'N/A'}}</td>
                        <td>{{$researches->submitted_to->name ?? 'N/A'}}</td>
                        <td>{{$researches->deleted_at->format("m/d/Y")}}<br><small>{{$researches->created_at->format("h:i A")}}</small></td>
                        <td>
