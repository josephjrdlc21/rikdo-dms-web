    ...
    <form method="GET" action="">
        <div class="card-header">
            ...
            <div class="card-header-action">
                ...
                <a href="{{route('portal.cms.research_types.index')}}" class="btn btn-sm btn-secondary" style="border-radius: 0.25rem !important;">Reset Filter</a>
            </div>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-sm-12 col-lg-5">
                    <div class="form-group">
                        ...
                        <input type="text" id="input_keyword" class="form-control" placeholder="eg. Type" name="keyword" value="{{$keyword}}">
                    </div>
                </div>
            ...
            <table class="table table-striped mb-0">
                <thead>
                    <tr>
                        <th>No.</th>
                        <th>Research Type</th>
                        <th class="text-center">Max Chapter</th>
                        <th>Date Added</th>
                        <th>Last Modified</th>
                        <th></th>
                    </tr>
                ...
                <tbody>                         
                    @forelse($record as $index => $research_type)
                    <tr>
                        <td>{{$loop->index + $record->firstItem()}}</td>
                        <td>{{$research_type->type}}</td>
                        <td class="text-center">{{$research_type->max}}</td>
                        <td>{{$research_type->created_at->format("m/d/Y h:i A")}}</td>
                        <td>{{$research_type->updated_at->format("m/d/Y h:i A")}}</td>
                        <td>
                            <div class="btn-group mb-2">
                                ...
                                <div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(0px, 29px, 0px); top: 0px; left: 0px; will-change: transform;">
                                    @if($auth->canAny(['portal.cms.research_types.update'], 'web'))         
                                    <a class="dropdown-item" href="{{route('portal.cms.research_types.edit', [$research_type->id])}}">Edit Details</a>
                                    @endif
                                    @if($auth->canAny(['portal.cms.research_types.delete'], 'web'))         
                                    <a class="dropdown-item delete-record" data-url="{{route('portal.cms.research_types.delete', [$research_type->id])}}" type="button" style="cursor: pointer;">Delete Type</a>
                                    @endif
                                </div>
                            </div> 
                        </td>
